  ...
  return (
    <aside className="fixed left-0 top-0 z-40 flex h-screen w-[240px] flex-col border-r border-border/70 bg-white/50 backdrop-blur-sm">
      <div className="flex h-[80px] items-center px-6 border-b border-border/50">
        <Link href="/dashboard" className="text-xs uppercase tracking-[0.35em] text-foreground/70 font-heading hover:text-foreground transition-colors duration-300">Dashboard</Link>
      </div>
      ...
      <div className="border-t border-border/50 p-3">
        <form action={logout}>
          <button type="submit" className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-foreground/50 hover:text-foreground hover:bg-foreground/5 transition-all duration-200">
