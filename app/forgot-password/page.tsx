...
import { resetPassword as resetPasswordAction } from "@/lib/supabase/actions";
...
async function wrappedResetPassword(_prevState: State, formData: FormData): Promise<State> {
  return await resetPasswordAction(formData);
}

export default function ForgotPasswordPage() {
  const [state, formAction, pending] = useActionState(wrappedResetPassword, initialState);
      ...
      <div className="flex min-h-screen flex-col items-center justify-center bg-background px-6 py-12">
        <div className="w-full max-w-sm text-center">
          ...
          <p className="text-sm text-foreground/60 leading-7">
            If an account exists with that email, we&rsquo;ve sent a password
